<?php   $Glo_PathRel_back = '../';
        include $Glo_PathRel_back.'../share/php/NSOSP.php'; ?>


<?php FrQFT_f_HTML_BeginHeaderBodyContainer( 'OM:NPYo:Chakras-Kabbala-Tarot'); ?>


      <?php FrQFT_f_Div_Header( '      '); ?>


      <?php FrQFT_f_Div_WrapperBegin( '      ', 'OM:NPYo:Home'); ?>

	
          <?php To_f_Paragraph_list( $Sc_g_Text_replace_ary, $Sc_g_Text_replace_preg_ary, '                ',
                array(
                  array( 'notice', array( Display => 'showContent', text => array(
                    '\\bold{Major Arcana (Fool\'s or Soul\'s Journey)}',
                    '\\bold{\\italic{Microcosmic Arcana (innere)}}',
                    '• Wurzel-Chakra oder Muladhara (Tarot: 10. Das Rad des Schicksals Karte, Wheel of Fortune Card)',
                    '• Sakral-Chakra oder Svadishtana (Tarot: 9. Der Eremit, The Hermit Card)',
                    '• Solarplexus-Chakra oder Manipura (Hara) (Tarot: weiblicher Aspekt, 8. Die Kraft, Strength; 7. männlicher Aspekt, Der Wagen, The Chariot)',
                    '• Herz-Chakra oder Anahata (Tarot: 6. Die Liebenden Karte, The Lovers Card, Tifereth Card)',
                    '⋅ Dieses Chakra und diese Karte stehen in meinen Augen nicht für verliebt sein, sondern dafür, sich um die Dinge zu kümmern (to care), die wir lieben. Das sind wir selber und unsere Liebsten, aber auch unsere Berufung, unsere Leidenschaften, unsere Mitmenschen und unsere Umwelt und so fort.',
                    '⋅ Körperlich geht es für mich nicht nur um den Herzschlag, der unseren Körper versorgt, weibliches Prinzip, sondern auch um unsere Atmung, die das Herz-Chakra öffnet und eine versorgende Verbindung zwischen innen und außen herstellt, männliches Prinzip. Der Herzschlag gehört ins Unterbewusste oder Unbewusste und das Herz sitzt auch in der linken Körperhälfte, weibliches Prinzip. Die Lunge ist auf der männlichen Handlungsseite unseres Körpers größer, als auf der weiblichen Seite. Wir können unsere Atmung zum Teil bewusst steuern, männliches Prinzip. Daher hat auch das Herz-Chakra, die Lovers Card, einen weiblichen und einen männlichen Aspekt.',
                    '• Hals-Chakra oder Vishudda (Tarot: weiblicher Aspekt, 5. Der Hierophant Karte, Der Papst Karte, The Hierophant Card, The Pope Card; 4. männlicher Aspekt, Der Herrscher Karte, The Emperor Card)',
                    '• Stirn-Chakra oder Ajna (Tarot: weiblicher Aspekt, 3. Die Herrscherin Karte, The Empress Card; 2. männlicher Aspekt, Die Hohepriesterin, Die Päpstin Karte, The High Priestess Card)',
                    '• Kronen-Chakra oder Sahasrara (Tarot: 1. Der Magier Karte, The Magician Card, The Magus Card or The Juggler Card)',
                    '• ???-Chakra oder ??? (Tarot: 0. (oder 11.?) oder 22. Der Narr Karte, Der Joker Karte, The Fool or The Soul Card (Mark Passio))',
                    '\\bold{\\italic{Macrocosmic Arcana (äußere)}}',
                    '• XXX',
                    '\\bold{Minor Arcana}',
                    '• XXX',
                    '\\bold{Wochentage sind nach Planeten und Göttern benannt, die den Chakras entsprechen}',
                    '• Siehe \\jump[https://de.wikipedia.org/w/index.php?title=Wochentag&oldid=232562969]{}{Wikipedia: Wochentag}.',
                    '\\bold{Chakras}',
                    '• Siehe Bild, was über Hannes Heine kam: „Chakrensystem-screenshot-2023-11-27-at-165906.png“.',
                    '– Vielleicht bei „@wingedshoespublishing“ um Genehmigung der Benutzung fragen?',
                    ))),
                      
                  array( 'text', array( text => array(
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n"))),
                  array( 'headline', array( jump_name => 'OM:NPYo:Chakras-Kabbala-Tarot:Vortext:XXX', text =>
                      
                'XXX', subline =>
                  '')),
                  array( 'text', array( text => array(
                    '\\color{*Bearb}{In Arbeit …}'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n"))),
                      
                  array( 'jumplist', array(
                      //array(  jump_name => 'OM:NPYo:XXX'),
                    )),
                )
          ); ?>
		    	<br>
          <?php To_f_headline_add_hides_end_line(); ?>
          

		    	<!  • XXX  >
          <?php $Sc_g_equation_auto_num = 0; ?>
          <?php To_f_Chapter_v1( $Sc_g_Text_replace_ary, $Sc_g_Text_replace_preg_ary, '          ', 'hideContent',
            'OM:NPYo:Chakras-Kabbala-Tarot:XXX',
              '\\color{*Bearb}{In Arbeit …}', 'Sc_f_Paragraph',
                array(
                  array( 'jumplist', array(
                      //array(  jump_name => 'OM:NPYo:XXX', type => 'back'),
                    )),
                      
                  array( 'notice', array( Display => 'showContent', text => array(
                    '• XXX',
                    ))),
                      
                  array( 'text', array( text => array(
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n"))),
                  array( 'headline', array( jump_name => 'OM:NPYo:Chakras-Kabbala-Tarot:XXX:XXX', text =>
                      
                'XXX', subline =>
                  '')),
                  array( 'text', array( text => array(
                    '\\color{*Bearb}{In Arbeit …}'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n".
                    'XXX'."\n"))),
                      
                  array( 'jumplist', array(
                      //array(  jump_name => 'OM:NPYo:XXX'),
                    )),
              )
          ); ?>


			<?php FrQFT_f_Div_WrapperEnd( '      '); ?>

	
<?php FrQFT_f_HTML_EndDivsNavExtrFootContainerBody(); ?>
